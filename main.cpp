...
FFTRegistration::~FFTRegistration()
{
}
...
void FFTRegistration::processImage_(const cv::Mat &gray0_, const cv::Mat &gray1_)
{
    ...
    cv::Mat FFT1_, FFT2_;
    // cv::Mat FFT1_ = cv::Mat::zeros(FFT1.rows, FFT1.cols, CV_32FC1);
    // cv::Mat FFT2_ = cv::Mat::zeros(FFT2.rows, FFT2.cols, CV_32FC1);
...
//    char input_name[] = "/Users/jin/Desktop/CMU/BIG/probe/apriltest.MOV";
    ...
    std::cout<<"Total frame #: "<<cap.get(cv::CAP_PROP_FRAME_COUNT)<<std::endl;
    if(!cap.isOpened()){
