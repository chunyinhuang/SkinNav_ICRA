...
#define FRAME_COUNT 1500 //1600, 4000, 4000
...
int main()
{
    ...
    double f_init =  1610.51324, cx_init = 954.94718, cy_init = 548.71677, Z_init = 2, Z_limit = 100, ba_loss_width = 9; // Negative 'loss_width' makes BA not to use a loss function.
...
//        imshow("color0", color0_);
        ...
        for(int rid=0; rid<RECONSTRUCT_INTERVAL; rid++){
            ...
            if(SHOW_FEATURE_TRACKING){
                ...
                {
                    circle( color0, p_corner[i], 5, cv::Scalar(255, 0, 0), -1);
                    circle( color1, q_corner[i], 5, cv::Scalar(255, 0, 0), -1);
                }
                ...
                imwrite("/Users/jin/Desktop/CMU/BIG/thesis/pic/p_corners.png", color0);
                imwrite("/Users/jin/Desktop/CMU/BIG/thesis/pic/q_corners.png", color1);
                cv::waitKey();
            }
...
//                Xs.push_back(cv::Point3d(0, 0, Z_init));
    ...
    //------------------------------------- Below is for reverse play of the video ----------------------------------------
    
    
   while(cap.isOpened() && count>INIT_FRAME)
   {
...
//                Xs.push_back(cv::Point3d(0, 0, Z_init));
    ...
    FILE* fpts = fopen("./reconstruction_points.xyz", "wt");
...
//        if (!is_noisy[i])
    ...
    FILE* fcam = fopen("./reconstruction_cameras.xyz", "wt");
